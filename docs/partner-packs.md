...
Model 2 pages and quick-access panes declare each displayed reading through a named `dataBindings` entry. A binding selects an approved semantic data key. Battery and device-specific keys are compiled with installation-resolved selectors and allow-listed measurement paths; aggregate battery, solar, load, tank and GX-relay keys resolve through GUIv2's system models. Partner QML consumes bindings through `PartnerMappedMetric`, which exposes availability, value, unit and resolved common name without accepting partner-authored D-Bus paths.
