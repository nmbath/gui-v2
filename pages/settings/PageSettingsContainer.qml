...
Page {
	...
	GradientListView {
		model: VisibleItemModel {
			...
			// The concrete fix for PortConflict (28, venus-containers repo
			// enums.py): another container already has this port, so -
			// simple-user-facing framing, not "resolve the conflict" jargon -
			// just move this one to a free port instead. Only this
			// container's first published port is covered today (see
			// reconciler.py's update_network_port docstring). Writing it
			// recreates the container immediately on the backend side, no
			// separate restart action needed here.
			ListSpinBox {
				//% "Change port to"
				text: qsTrId("pagesettingscontainer_port_conflict_change_port")
				dataItem.uid: root.containerPrefix + "/Network/Port/0/HostPort"
				from: 1
				to: 65535
				preferredVisible: errorCode.value === 28 // ErrorCode.PortConflict
			}
