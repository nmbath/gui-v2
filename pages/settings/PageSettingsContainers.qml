...
Page {
	...
	ExchangeAction {
		...
		confirmDescription: qsTrId("pagesettingscontainerimport_confirm_description")
		//% "Adding container"
		processingText: qsTrId("pagesettingscontainerimport_adding")
		//% "Container added."
	...
	VeQItemSortTableModel {
		...
		model: VeQItemTableModel {
			uids: [BackendConnection.serviceUidForType("storage") + "/Volumes"]
	...
	Connections {
		target: selectedStorage
		function onValueChanged() { root.recomputeSelectedStorage() }
	}

	// Local clock for ticking a Creating/Recreating row's elapsed-pull time
	// client-side, the same pattern PageSettingsExchange.qml already
	...
	GradientListView {
		model: VisibleItemModel {
			...
			PrimaryListLabel {
				//% "Local connection required to add a container from a file."
				text: qsTrId("pagesettingscontainers_vrm_upload_unavailable")
				preferredVisible: enabledSwitch.checked && BackendConnection.vrm
						&& containerImportAction.serviceConnected
						&& containerImportAction.actionAvailable
			}

			SettingsListHeader {
				//% "Storage"
				text: qsTrId("pagesettingscontainers_storage")
				preferredVisible: enabledSwitch.checked && selectedStorage.valid
			}

			ListNavigation {
				//% "Container storage"
				text: qsTrId("pagesettingscontainers_container_storage")
				secondaryText: {
					if (!selectedStorage.value) {
						//% "No volume selected"
						return qsTrId("pagesettingscontainers_no_storage_selected")
					}
					...
					//% "Unnamed volume"
					return root.selectedStorageLabel || qsTrId("pagesettingscontainers_container_storage_unnamed")
				}
				caption: {
					if (storageSelectionError.value) {
						return storageSelectionError.value
					}
					if (root.selectedStorageIsLocalData) {
						//% "%1 used / %2 free"
			...
			SettingsColumn {
				...
				Repeater {
					...
					delegate: ListNavigation {
						...
						// Same left-edge indicator ListSetting already uses for the
						// access-level bar (e.g. Settings > System status) - reuses that
						// existing affordance for severity instead of a dot buried
						// mid-row, transparent for the common case of no error/warning.
						indicatorColor: Containers.severityColor(state.value, errorCode.value, dbusState.value)
						...
						// Centre the resource usage and chevron against the list item
